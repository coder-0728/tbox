...
int aes_ecb_encrypt(const unsigned char *key, const unsigned char *input, int input_length, unsigned char *output)
{
    ...
    {
        LOG_Error("aes_ecb_encrypt input failed");
        return RET_ERR;
    }
...
int aes_cbc_128_encrypt_no_padding(unsigned char* plaintext, int plaintext_len, unsigned char* key, unsigned char* iv, unsigned char* ciphertext)
{
    AES_KEY aes_key;
